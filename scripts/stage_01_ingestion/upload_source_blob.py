...
def verify_remote_blob(blob_client: object, metadata: dict[str, object]) -> None:
    ...
    if properties.size != metadata["size_bytes"]:
        raise RuntimeError("Remote blob size does not match the registered source")
    if properties.content_settings.content_type != metadata["content_type"]:
        raise RuntimeError("Remote Content-Type does not match the source")
    if properties.metadata.get("sha256") != metadata["sha256"]:
        raise RuntimeError("Remote SHA-256 metadata does not match the source")
...
def upload_and_verify(source_paths: list[Path]) -> None:
    ...
    try:
        for source_path in source_paths:
            ...
            action = (
                "uploaded and verified" if uploaded else "already present and verified"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Upload a registered local PDF or HTML source."
    )
    parser.add_argument(
        "--file",
        nargs="+",
        required=True,
        help="Source path inside data/sources, relative to the project root.",
    )
...
def main() -> None:
    args = parse_args()
    source_paths = [resolve_source_document(value) for value in args.file]
    upload_and_verify(source_paths)
